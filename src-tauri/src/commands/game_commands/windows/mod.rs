...
/// ReturnOfModding remembers whether its first successful launch used an
/// explicit root. If that launch came from r2modman, omitting the same signal
/// later can make the loader disable itself. A modded launch therefore always
/// declares the tree that r2modmac applied.
fn return_of_modding_arguments(
    game_path: &std::path::Path,
    prefix_root: Option<&std::path::Path>,
) -> Vec<String> {
    let root = prefix_root
        .and_then(|prefix| map_native_path_to_wine_path(prefix, game_path))
        .unwrap_or_else(|| game_path.to_string_lossy().to_string());
    vec!["--rom_modding_root_folder".to_string(), root]
}

fn windows_modded_arguments(
    game_path: &std::path::Path,
    shimloader: Option<&ShimloaderLaunch>,
    enable_return_of_modding: bool,
    prefix_root: Option<&std::path::Path>,
) -> Vec<String> {
    let mut arguments = shimloader
        .map(|launch| launch.arguments(prefix_root))
        .unwrap_or_default();
    if enable_return_of_modding {
        arguments.extend(return_of_modding_arguments(game_path, prefix_root));
    }
    arguments
}
...
pub(crate) fn launch_windows_direct_game(
    ...
    launch_windows_direct_game_with_working_dir(
        game_path,
        None,
        shimloader,
    ...
    #[cfg(unix)]
    {
        ...
        if let Some(prefix_root_path) = prefix_root.as_deref() {
                ...
                find_macos_wineskin_launcher_binary(Some(prefix_root_path), &executable_path)
            {
                ...
                let arguments = windows_modded_arguments(
                    game_path,
                    shimloader,
                    enable_return_of_modding,
...
mod tests {
    ...
    #[cfg(unix)]
    #[test]
    fn return_of_modding_gets_an_explicit_wine_root_with_spaces() {
        let root = std::env::temp_dir().join(format!(
            "r2modmac-rom-root-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
        ...
        assert_eq!(
            windows_modded_arguments(&ship, None, true, Some(&prefix)),
            vec![
...
pub(crate) fn launch_windows_game(
    ...
    match plan_windows_launch(&steam_roots, game_path) {
        ...
        WindowsLaunchPlan::Direct => {
            launch_windows_direct_game(game_path, shimloader, enable_return_of_modding)
        }
