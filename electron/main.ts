...
ipcMain.handle('fetch-communities', async () => {
    try {
        ...
        let allResults: any[] = []
        let nextUrl: string | null = 'https://thunderstore.io/api/experimental/community/'

        while (nextUrl) {
            console.log(`Fetching communities from: ${nextUrl}`)
            const data: any = await fetchPage(nextUrl)
...
ipcMain.handle('delete-profile-folder', async (_, profileId: string) => {
    const profileDir = path.join(app.getPath('userData'), 'profiles', profileId);
    if (fs.existsSync(profileDir)) {
...
ipcMain.handle('open-mod-folder', async (_, { profileId, modName }: { profileId: string, modName: string }) => {
