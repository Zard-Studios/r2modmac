...
usage() {
  cat <<'USAGE'
Usage: ./build-local.sh [all|macos|linux|linux-x64|linux-arm64] [--skip-checks] [--clean]

Build shipping artifacts into dist-local/.

  all          Build macOS (when running on macOS) and both Linux architectures.
  macos        Build Apple Silicon and Intel macOS DMGs.
  linux        Build Linux x64 and ARM64 tarballs through Docker Buildx.
  linux-x64    Build only r2modmac_linux_x64.tar.gz.
  linux-arm64  Build only r2modmac_linux_arm64.tar.gz.

Options:
  --skip-checks  Skip format, Rust tests, and Cargo check. Frontend compilation still runs.
  --clean        Remove dist-local/ and local target caches before building.
USAGE
}
...
for arg in "$@"; do
  case "$arg" in
    all|macos|linux|linux-x64|linux-arm64) MODE="$arg" ;;
...
verify_sponsor_endpoint() {
  require curl
  local status
  # A GET is deliberately route-rejected with 204 by the Worker and never calls the ad SDK.
...
build_macos_target() {
  ...
  # Shipping builds always use the production fallback compiled into the app.
  ...
  [[ -f UNVERIFIED_APP_INSTRUCTIONS.txt ]] \
    && cp UNVERIFIED_APP_INSTRUCTIONS.txt "$dmg_root/"
...
build_linux_target() {
  ...
  docker run --rm \
    ...
    bash -lc '
      ...
      # POSIX ustar plus a de-sparsified staging copy prevents GNUSparseFile.0 pseudo-paths.
...
build_linux_x64() {
  build_linux_target "linux/amd64" "x64" "X86-64"
}

build_linux_arm64() {
  build_linux_target "linux/arm64" "arm64" "AArch64"
}
...
if [[ "$SKIP_CHECKS" == "0" ]]; then
  ...
  warn "Shipping checks were skipped by explicit request."
fi
