...
export const ProfileSidebar: React.FC<ProfileSidebarProps> = ({
    ...
    } else if (isCancellingLaunch) {
        launchActionTitle = 'Stopping the launch...';
    } else if (launchIsCancellable) {
        launchActionTitle = 'Waiting for the game to start — click to stop waiting';
    ...
    const launchActionButton = (
        <button
            onClick={() => {
                ...
                if (launchIsCancellable) {
                    void onCancelLaunch?.();
                    return;
                }
                void onLaunchProfile();
            }}
            disabled={launchIsCancellable
                ? false
                : isLaunching || isApplying || isSteamRestarting || launchActionBlocked}
            ...
            {runtimeHealth && !runtimeQueuedForSync && ['missing', 'incomplete', 'unconfigured', 'unsupported'].includes(runtimeHealth.status) && (
                <div className="profile-sidebar-motion-item [--sidebar-motion-order:2] mx-4 mb-2 flex items-center gap-2 rounded-xl border border-amber-500/30 bg-amber-500/10 px-3 py-2.5">
                    ...
                    {runtimeHealth.status === 'unsupported' ? null : <button
                        ...
                        disabled={isRepairingRuntime || (runtimeHealth.status !== 'unconfigured' && !runtimeHealth.repairable && !runtimeHealth.missingComponents.includes('profile-location'))}
