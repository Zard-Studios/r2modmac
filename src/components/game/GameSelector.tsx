...
const GameCard = memo(function GameCard({
    ...
    return (
        <button
            ...
            // transition-[border-color,box-shadow,transform] instead of transition-all
            // avoids triggering layout/paint for ALL properties on ~600 nodes simultaneously
        ...
        >
            ...
            <div className="aspect-[3/4] w-full relative overflow-hidden bg-gray-950" style={{ contain: 'paint' }}>
                ...
                {/* Platform badge — solid dark bg replaces backdrop-blur-md.
                    backdrop-blur forces a GPU compositor layer on every single card which kills scroll FPS. */}
                <div className="absolute top-2 right-2 flex flex-col gap-1 z-30 pointer-events-none items-end">
                    <div className={`bg-[rgba(0,0,0,0.72)] text-white p-1 rounded-full shadow-lg border border-white/10 flex items-center justify-center gap-2 h-7 ${isMacCompatible ? 'px-2' : 'w-7 px-0'}`}>
                        ...
                        {isMacCompatible && (
                            <>
                                {isWindowsCompatible && <div className="w-[1px] h-3.5 bg-white/20" />}
